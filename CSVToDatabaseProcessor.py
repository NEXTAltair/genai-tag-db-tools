...
class CSVToDatabaseProcessor:
    ...
    def process_row(self, row: dict, format_id: int, default_type_id: int):
        ...
        self.tag_status[(tag_id, format_id)] = (type_id, current_status[1], current_status[2])
    ...
    def get_default_type_id(self, filename: str) -> int:
        ...
        for type_name, type_id in {**danbooru_types, **e621_types, **derpibooru_types}.items():
    ...
    def insert_into_final_tables(self):
        ...
        try:
            ...
            VALUES (?, ?, ?)
            ''', [(tag_id, source_tag, normalized_tag) for tag_id, (source_tag, normalized_tag) in self.tags.items()])
            ...
            status_data = [
                ...
                for (tag_id, format_id), (type_id, alias, preferred_tag_id) in self.tag_status.items()
            ]
            cursor.executemany('''
    ...
    def deduplicate_data(self):
        ...
        # tag_statusの整理と検証
        inconsistencies = []
        for (tag, format_id), (type_id, alias, preferred_tag) in self.tag_status.items():
    ...
    def _export_tag_status(self, file_path: Path):
        with file_path.open('w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["tag_id", "format_id", "type_id", "alias", "preferred_tag"])
            for (tag_id, format_id), (type_id, alias, preferred_tag) in self.tag_status.items():
