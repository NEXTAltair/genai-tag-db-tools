...
def test_tags_table(tags_db, original_df):
    ...
    ), f"❌ 以下のタイトルがTAGSテーブルに存在しません: {missing_titles}"
...
def test_translations_table(translations_db, tags_db, original_df):
    ...
    df_translations = original_df.explode("other_names").select(
        ["id", "title", "other_names"]
    )
    ...
    df_translations = df_translations.join(
        title_tag_id_df.select(["title", "tag_id"]), on="title", how="left"
    )

    # 期待される翻訳のDataFrameを作成
    expected_translations = df_translations.rename(
        {"other_names": "translation"}
    ...
    assert (
        len(missing_tag_ids) == 0
    ), f"❌ 以下のタイトルのtag_idが見つかりません: {missing_tag_ids}"

    # TAGSとTAG_TRANSLATIONSテーブルをJOINしてデータを取得
    joined_db = (
        tags_db.join(translations_db, on="tag_id", how="inner")
        .filter(pl.col("language") == "japanese")
        .select(["tag_id", "translation"])
    )

    # 欠けている翻訳を見つける
    missing_translations = expected_translations.join(
        joined_db, on=["tag_id", "translation"], how="anti"
    )

    if len(missing_translations) > 0:
        print(
            "⚠️ TAG_TRANSLATIONS か danbooru-ja-tag-pair-20241015 テーブルに以下の翻訳が存在しません:"
        )
        print(missing_translations)

    # danbooru-ja-tag-pair-20241015 にある情報が TAG_TRANSLATIONS に登録されていない場合のみエラーとする
    missing_in_tags_db = expected_translations.join(
        joined_db, on=["tag_id", "translation"], how="anti"
    )
    assert (
        len(missing_in_tags_db) == 0
    ), f"❌ danbooru-ja-tag-pair-20241015 テーブルに存在するが TAG_TRANSLATIONS テーブルに存在しない翻訳があります: {missing_in_tags_db}"
...
def test_tag_status_table(status_db, tags_db, original_df):
    ...
    type_id_df = pl.DataFrame(
        {"type": list(type_id_map.keys()), "type_id": list(type_id_map.values())}
    )
    ...
    # タイトルをtag_idにマッピング（joinを使用）
    title_tag_id_df = tags_db.rename({"source_tag": "title"})
    expected_statuses = expected_statuses.join(
        title_tag_id_df.select(["title", "tag_id"]), on="title", how="left"
    )
    ...
    # 必要な列を選択
    expected_statuses = expected_statuses.select(
        ["tag_id", "format_id", "type_id"]
    ).drop_nulls("tag_id")

    # データベースから実際のタグステータスを取得し、format_idのデータ型をi64に統一
    actual_statuses = status_db.with_columns(pl.col("format_id").cast(pl.Int64)).select(
        ["tag_id", "format_id", "type_id"]
    )

    # 欠けているまたは不正なタグステータスを見つける
    missing_statuses = expected_statuses.join(
        actual_statuses, on=["tag_id", "format_id", "type_id"], how="anti"
    )

    if len(missing_statuses) > 0:
        print(
            "❌ TAG_STATUSテーブルに以下のタグステータスが存在しません、または不正です:"
        )
        print(missing_statuses)
    assert (
        len(missing_statuses) == 0
    ), f"❌ TAG_STATUSテーブルに以下のタグステータスが存在しません、または不正です: {missing_statuses}"
