...
class DatabaseMaintenanceTool:
    ...
    def detect_duplicates_in_tag_status(self) -> list[dict[str, Any]]:
        ...
        all_statuses = self.reader.list_tag_statuses()
        ...
        for (tag_id, format_id), statuses in status_groups.items():
            ...
            tag = self.reader.get_tag_by_id(tag_id)
            if not tag:
                continue
    ...
    def detect_usage_counts_for_tags(self) -> list[dict[str, Any]]:
        ...
        tag_ids = self.reader.get_all_tag_ids()
        format_ids = self.reader.get_tag_format_ids()
        ...
        for tag_id in tag_ids:
            ...
            for format_id in format_ids:
                count = self.reader.get_usage_count(tag_id, format_id)
                if count:
                    format_name = self.reader.get_format_name(format_id)
                    usage_counts.append(
                        {
    ...
    def detect_foreign_key_issues(self) -> list[tuple[int, str | None]]:
        ...
        missing_tags = []
        for status in all_statuses:
            tag = self.reader.get_tag_by_id(status.tag_id)
            if not tag:
    ...
    def detect_orphan_records(self) -> dict[str, list[tuple[int]]]:
        """孤立レコードを検出する（簡易版）。"""
        all_tag_ids = set(self.reader.get_all_tag_ids())
        ...
        for tag_id in all_tag_ids:
            translations = self.reader.get_translations(tag_id)
    ...
    def detect_missing_translations(
        ...
        if required_languages is None:
            required_languages = set(self.reader.get_tag_languages())

        missing_translations: list[dict[str, Any]] = []
        for tag_id in self.reader.get_all_tag_ids():
            tag = self.reader.get_tag_by_id(tag_id)
            if not tag:
                continue
    ...
    def detect_abnormal_usage_counts(self, max_threshold: int = 1_000_000) -> list[dict[str, Any]]:
        ...
        for tag_id in self.reader.get_all_tag_ids():
            for format_id in self.reader.get_tag_format_ids():
                count = self.reader.get_usage_count(tag_id, format_id)
    ...
    def detect_invalid_tag_id(self) -> int | None:
        ...
        invalid_tag_id = self.reader.get_tag_id_by_name("invalid tag")
        if invalid_tag_id is None:
            invalid_tag_id = self.reader.get_tag_id_by_name("invalid_tag")
    ...
    def fix_inconsistent_alias_status(self, status_id: tuple[int, int]) -> None:
        ...
        status = self.reader.get_tag_status(tag_id, format_id)
        if not status:
            return
    ...
    def fix_duplicate_status(self, tag_id: int, format_id: int) -> None:
        """重複したタグステータスを修正する。"""
        statuses = self.reader.list_tag_statuses(tag_id)
