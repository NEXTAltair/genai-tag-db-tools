...
class TagStatisticsWidget(QWidget, Ui_TagStatisticsWidget):
    ...
    def calculate_statistics(self):
        ...
        self.statistics["formats"] = (
            self.tag_data["formats"].str.split(",", expand=True).stack().value_counts()
        )
        self.statistics["types"] = (
            self.tag_data["types"].str.split(",", expand=True).stack().value_counts()
        )
        self.statistics["languages"] = (
            self.tag_data["translations"]
            .str.split(",", expand=True)
            .apply(lambda x: x.str.split(":").str[0])
            .stack()
            .value_counts()
        )
        self.statistics["top_tags"] = self.tag_data.nlargest(10, "total_usage_count")[
