...
from genai_tag_db_tools.config import engine, DF_SCHEMA, AVAILABLE_COLUMNS
...
class TagDatabase:
    ...
    def init_tagtypeformatmapping(self):
        ...
        initial_data = [
            TagTypeFormatMapping(
                format_id=0, type_id=0, type_name_id=0, description=""
            ),
            TagTypeFormatMapping(
                format_id=1, type_id=0, type_name_id=1, description=""
            ),
            TagTypeFormatMapping(
                format_id=1, type_id=1, type_name_id=2, description=""
            ),
            TagTypeFormatMapping(
                format_id=1, type_id=3, type_name_id=3, description=""
            ),
            TagTypeFormatMapping(
                format_id=1, type_id=4, type_name_id=4, description=""
            ),
            TagTypeFormatMapping(
                format_id=1, type_id=5, type_name_id=7, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=6, type_name_id=6, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=7, type_name_id=7, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=8, type_name_id=8, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=1, type_name_id=2, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=0, type_name_id=1, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=5, type_name_id=5, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=3, type_name_id=3, description=""
            ),
            TagTypeFormatMapping(
                format_id=2, type_id=4, type_name_id=4, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=5, type_name_id=10, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=6, type_name_id=11, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=7, type_name_id=7, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=8, type_name_id=12, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=9, type_name_id=13, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=10, type_name_id=14, description=""
            ),
            TagTypeFormatMapping(
                format_id=3, type_id=11, type_name_id=16, description=""
            ),
        ]
        session = self.create_session()
        for data in initial_data:
            existing = (
                session.query(data.__class__)
                .filter_by(
    ...
    def init_tagtypename(self):
        ...
        for data in initial_data:
            existing = (
                session.query(data.__class__)
                .filter_by(
                    type_name_id=data.type_name_id,
                    type_name=data.type_name,
    ...
    def dataframe_schema(self):
        """テスト用 データフレームのスキーマを表示する"""
        for table_name, df in DF_SCHEMA.items():
            print(f"Table: {table_name}")
            print(df.schema)

    def get_tags_table_top10(self):
        """テスト用 TAGS テーブルの冒頭10件を取得する"""
...
class Tag(Base):
    ...
    __table_args__ = (UniqueConstraint("tag", "source_tag"),)
...
class TagFormat(Base):
    ...
    __table_args__ = (UniqueConstraint("format_name"),)
...
class TagStatus(Base):
    ...
    alias: Mapped[bool] = mapped_column(Boolean)
    preferred_tag_id: Mapped[int] = mapped_column(ForeignKey("TAGS.tag_id"))

    # back_populatesで相互参照を設定
    tag: Mapped["Tag"] = relationship(
        foreign_keys=[tag_id],
        back_populates="statuses",  # Tagクラスのstatusesと対応
    )
    preferred_tag: Mapped["Tag"] = relationship(foreign_keys=[preferred_tag_id])
